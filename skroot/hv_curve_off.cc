...
int main(int argc, char *argv[]) {
    ...
    Int_t PMTinfo[MAXPM][5] = {0};//flags, hv, x, y, z
    ...
    if (connect.is_open()){
	    ...
	    PMTinfo[cableid-1][2]=pmtx;
	    PMTinfo[cableid-1][3]=pmty;
	    PMTinfo[cableid-1][4]=pmtz;
	    if (cableid == 7107) PMTinfo[cableid-1][1] = hvswap2;
	    else if (cableid == 7059) PMTinfo[cableid-1][1] = hvswap1;
        }
    ...
    PMTinfo[7106][1] = hvswap1;
    PMTinfo[7058][1] = hvswap2;
    ...
    //for (int iswap = 0; iswap < nfile; iswap++){
    //Double_t temphv;
    //temphv = skhv[iswap][7106];
    //skhv[iswap][7106] = skhv[iswap][7058];
    //skhv[iswap][7058] = temphv;
    //}
    //fsyscheck->Close();
    ...
    for (Int_t p = PlotRange[0]; p < min(MAXPM, PlotRange[1]); p++){
	  ...
	  if (skcable[0][p]>0) outtxt_target <<  skcable[0][p] << setw(8) << "      -1" << setw(8) << "      -1" << endl;
	  else if (skhv[3][p]>=0) outtxt_target << p+1 << setw(8) << setprecision(2) << fixed << skhv[3][p] << setw(8) << setprecision(2) << fixed << skhv[3][p] << endl;
	...
	outtxt_target <<  skcable[0][p] << setw(8) << setprecision(2) << fixed << geighthv[ipmttype] << setw(8) << setprecision(2) << fixed << gfourhv[ipmttype] << endl;
           
	if (beta[ipmttype] <= indexmin){

	  fHVsk[p]->SetLineColor(2);
	  outtxt_badfit  << setw(10) << skcable[0][p] << setw(4) << " "+PMTtypeNames[ipmttype] << setw(12) << Form("%1.3e", gfourhv[ipmttype]) << setw(12) << Form("%1.3e", geighthv[ipmttype]) << setw(25) << Form("%1.2e(%1.2e)", norm[ipmttype], normerr[ipmttype]) << setw(20) << Form("%1.2f(%1.2f)", beta[ipmttype], betaerr[ipmttype]) << setw(15) << Form("%1.3f", rchi2[ipmttype] ) << setw(15) << Form("%1.2e", fHVsk[p]->GetProb()) << setw(15) << "     Flat_Curve" << "\n";
	}
    ...
    for (Int_t p = PlotRange[0]; p < min(MAXPM, PlotRange[1]); p++){
        ...
        if (TMath::Exp(fHVinvsk[p]->Eval(targetQ)) < 2450 && TMath::Exp(fHVinvskerr[p]->Eval(targetQ) < 2450)){

	t->AddText(Form("HV at 2.8e7 gain : %4.2f [V]", TMath::Exp(fHVinvsk[p]->Eval(targetQHPK))));
	  ...
	  t->AddText(Form("HV at 3.3e7 gain : %4.2f [V]", TMath::Exp(fHVinvsk[p]->Eval(targetQ))));      
	...
	else cout << "Target HV larger than 2450V for cable: " << skcable[0][p] << endl;
	c1->Modified();
	c1->Update();
	c1->Print(CanvasName);
    }
