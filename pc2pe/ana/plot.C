{
  ...
  for (int ifile=0; ifile<nFiles; ifile++) {
    ...
    ////////////////////////////////////////////
    // Plot On-time minus Off-time charge per channel for low intensity only
    if (ifile<3) {
  ...
  for (int ifile=0; ifile<nFiles/2; ifile++) {
    ...
    if (ifile==3) h_qmean[ifile]->GetYaxis()->SetRangeUser(-10, 120);
  ...
  for (int ifile=0; ifile<nFiles/2; ifile++) {
    ...
    if (!ifile) t_pc2pe->Branch("channel", &channel, "channel/I");
