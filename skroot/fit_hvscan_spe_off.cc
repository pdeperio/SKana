...
//const float xball = 35.35;
//const float yball = -70.7;
//const float zball = 0;
...
TF1* FitHK(TH1D* h, Int_t rebin = 2, Float_t hv = 0, Float_t PMTflag = 6){
    ...
    //TF1 *func2peak_bs = new TF1("f2peakbs","gaus(0)+0.5*[3]*[0]*((TMath::Erf(((x-[6])/[7])))+(1-TMath::Erf((x-[1]+0.5*[2])/[2]))-1)+0.25*[5]*[0]*((TMath::Erf((x-[1]-0.5*[2])/[2])+(1-TMath::Erf((x-[4]*[1])/[2])))-1)",0,12);
    //TF1 *func0peak = new TF1("fgaus0","gaus",-1, 12);0.5*[3]*[0]*((TMath::Erf(((x-[4])/[5])))
    TF1 *func2peak_bs = new TF1("f2peakbs","gaus(0)+0.5*[3]*[0]*((TMath::Erf(((x-[4])/[5]))+(1-TMath::Erf((x-[1])/[2]))-1))",0,12);
    TF1 *func1peak = new TF1("fgaus1","gaus",0, 12);
    TF1 *funcbs = new TF1("fbs","0.5*[3]*[0]*((TMath::Erf(((x-[4])/[5])))+(1-TMath::Erf((x-[1])/[2]))-1)",0, 12);
    ...
    if (hv >= -100){//fit 2peak+erf
        ...
        func2peak_bs->SetParName(4, "left cut-off");
        func2peak_bs->SetParName(5, "err func resolution");
        ...
        func2peak_bs->SetParameter(4, 0.3);
	func2peak_bs->SetParLimits(4, -1, 1);
	func2peak_bs->SetParameter(5, 0.3);
	func2peak_bs->SetParLimits(5, 0, func2peak_bs->GetParameter(2));

	func2peak_bs->SetRange(0, func1pe_new->GetParameter(1)+3*func1pe_new->GetParameter(2));
        h->Fit(func2peak_bs,"BQ+","", 0, func1pe_new->GetParameter(1)+3*func1pe_new->GetParameter(2));
	...
	//funcbs->FixParameter(7, func2peak_bs->GetParameter(7));
	
        //h->Fit(func0peak,"BQ+","", -1, 10);
	func1peak->SetRange(0, func1pe_new->GetParameter(1)+3*func1pe_new->GetParameter(2));
	funcbs->SetRange(0, func1pe_new->GetParameter(1)+3*func1pe_new->GetParameter(2));
        h->Fit(func1peak,"BQ+","", 0, func1pe_new->GetParameter(1)+3*func1pe_new->GetParameter(2));
        h->Fit(funcbs,"BQ+","", 0, func1pe_new->GetParameter(1)+3*func1pe_new->GetParameter(2));
...
TF1*/*std::vector<Double_t>*/ FitSK(TH1D* h, Int_t rebin = 2, Float_t hv = 0, Float_t PMTflag = 3){
    Int_t npx = 1000;
    Int_t entry = h->GetEntries();
    Int_t nbin = h->GetNbinsX();
    if (rebin > 1) h->Rebin(rebin);
    ...
    if (func1pe_new->GetParameter(2)>=1&&func1pe_new->GetParameter(2)/func1pe_new->GetParameter(1)>=0.2){//fit gaus + expo
	  ...
	  func2peak->SetParLimits(iSet, func1pe_new->GetParameter(iSet)-func1pe_new->GetParError(iSet), func1pe_new->GetParameter(iSet)+func1pe_new->GetParError(iSet));
        ...
        func2peak->SetParameter(3, 0.3);
        func2peak->SetParameter(4, 0.3);
        //func2peak->SetParLimits(3, 0.5, 10);
        func2peak->SetParLimits(3, 0, 1);
        //func2peak->SetParLimits(5, 0, func1pe_new->GetParameter(2));
        func2peak->SetParLimits(4, -1, 1);
	...
	//func2peak->SetParameter(6, 0.3);
	//func2peak->SetParLimits(6, 0, func2peak->GetParameter(2));
	func2peak->SetParameter(5, 1);
	func2peak->SetParameter(6, -3);
	...
	func1pe_new->Delete();
        ...
        funcskgaus->SetParameter(1, h->FindBin(h->GetMaximumBin()));
    ...
    //if (func->GetNpar() == 4) func->FixParameter(1, func->GetMaximumX(0,10));
	...
	myt[1] = new TLatex(0,0,Form("SPE Peak  = %1.2e [pC]",(func->GetParameter(1)>(func->GetMaximumX(0, 8)-0.5))?func->GetParameter(1):func->GetMaximumX(0,8)));
	//else if (func->Npar() == 4) myt[1] = new TLatex(0,0,Form("SPE Peak  = %1.2e [pC]",func->GetMaximumX(0,10)));
        myt[2] = new TLatex(0,0,Form("SPE Peak #sigma = %1.2e [pC]",func->GetParameter(2)));
        myt[3] = new TLatex(0,0,Form("Func Max = %1.2e [pC]",func->GetMaximumX(0,10)));
        myt[4] = new TLatex(0,0,Form("Chi2ndf  = %1.2e",func->GetChisquare()/func->GetNDF()));
        //if (func->GetNpar() > 3){
...
int main(int argc, char *argv[]) {// process the arguments
    ...
    if (connect.is_open()){
	    ...
	    if (cableid == 7107) hvswap1 = PMTinfo[7106][2];
	    else if (cableid == 7059) hvswap2 = PMTinfo[7058][2];
	}
        connect.close();
    }

    //PMTinfo[7106][2]=hvswap2;
    //PMTinfo[7058][2]=hvswap1;
    ...
    PMTinfo[7106][2]=hvswap2;
    PMTinfo[7058][2]=hvswap1;//the two ch hv cables are swapped
	...
	// For SK
	else if (iPMTtype==sk && (fge->GetParameter(1) < 0 || fge->GetParameter(1) < (fge->GetMaximumX() - 0.5) || fge->GetNpar() == 4)) {
